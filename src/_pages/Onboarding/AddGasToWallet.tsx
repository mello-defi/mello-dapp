...
export default function AddGasToWallet() {
  ...
  return (
    <div>
      <div
        className={
          'flex flex-col md:flex-row items-center space-x-0 md:space-x-2 space-y-2 md:space-y-0'
        }
      >
        <Button
          className={'w-full md:w-1/2'}
          variant={ButtonVariant.SECONDARY}
          onClick={() => setFundSource(FundSource.Fiat)}
        >
          Fund with card/bank transfer
        </Button>
        <Button
          className={'w-full md:w-1/2'}
          variant={ButtonVariant.SECONDARY}
          onClick={() => setFundSource(FundSource.Crypto)}
        >
          Fund step with crypto
        </Button>
      </div>
      <div className={'mt-2'}>
